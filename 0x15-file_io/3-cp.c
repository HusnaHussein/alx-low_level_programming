...
int main(int argc, char *argv[])
{
	...
	file_permn = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH;
	fd_to = open(argv[2], O_WRONLY | O_CREAT | O_TRUNC, file_permn);
	...
	while (lenrd == 1024)
	{
		...
		lenwt = write(fd_to, buffer, lenrd);
		if (lenwt != lenrd)
			lenwt = -1;
		check99(lenwt, argv[2], fd_from, fd_to);
	}
